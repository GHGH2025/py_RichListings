...
2. At **post selection**, the system resolves the URL, scrapes static/lazy/meta image URLs (including public Google Photos album tokens), and uses a rendered browser fallback only when the static page has no images, then creates `other_images_dropbox_link`.
